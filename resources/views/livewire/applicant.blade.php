...
<form action="{{ route('applicant.store') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="{{ $currentStep == 1 ? 'animate-slide-in-left' : 'hidden' }}">
        ...
        <div class="grid md:grid-cols-3 md:gap-6">
            <x-input.form span="Nombres Completos" name="nombres" model="applicant.nombres" disable="{{ $disable }}" />
    ...
    <div class="{{ $currentStep == 2 ? 'animate-slide-in-right relative' : 'hidden' }}">
        ...
        <div class="grid md:grid-cols-3 md:gap-6">
            <label class="block mb-10 relative">
                ...
                <input type="text" wire:model.debounce.500ms="searchSchoolName" wire:input="$set('showSchools', true)"
                    class="mt-1 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1"
            ...
            <label class="block mb-10">
                ...
                <select name="anno_egreso" wire:model="applicant.anno_egreso"
                    ...
                    <option class="hidden">Seleccionar</option>
                    @php
                    $maxYear = $applicant->modalidad_id != \App\Utils\Constants::MODALIDAD_QUINTO_SECUNDARIA ? date('Y')
                    - 1 : date('Y');
                    @endphp
                    @for ($i = $minimumYear; $i <= $maxYear; $i++) <option value="{{ $i }}">{{ $i }}</option>
                        @endfor
                </select>
                @else
